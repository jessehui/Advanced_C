...
int main(void)
{
	...
	OSTimeSet(0);
	//创建消息邮箱
	pmbox = OSMboxCreate(&KeyData);//参数为按键的键值 返回值为一个事件类型
	OSStart();
}
...
void beepTask(void *pdata)
{
	...
	{
		//等待接收到消息
		pkey = (INT8U *)OSMboxPend(pmbox,0,&err);//返回值是指向消息的指针 有几个按键被按下 pkey指向的值就为多少
		for (i=0;i<*pkey;i++)
		{	
		OPEN_BEEP();
	...
	while(1)
	{
		...
		if( (!KEY0) || (!KEY1) || (!KEY2))//按下变低电平
		{
			...
			if((!KEY0) || (!KEY1) || (!KEY2))//消抖
			{
				...
				//发送消息
				OSMboxPost(pmbox,&keyvalue);//返回值是错误码 可以不需要
				//等待按键抬起
